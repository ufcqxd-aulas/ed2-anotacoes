...
\begin{document}
  ...
  \part*{\provatitle}
    ...
    \subsection{Contagem de nós da árvore}
      ...
      Observe que a função \( N(h) \) é monotonicamente crescente\footnote{Isso porque todos os valores envolvidos são não-negativos e as operações realizadas são apenas adições.}. Com isso, e observando os casos-base, podemos afirmar que \( N(h) \geq 2N(h-2) \)\footnote{Considere que substituímos o termo \( N(h-1) \) por um termo \( N(h-2) \) e o termo 1 por um termo 0. Em ambas as substituições, os novos valores são menores.} para todo \( h \geq 3 \)\footnote{É válido para todo \( h \geq 2 \), mas o valor-base 0 atrapalha a argumentação que vem a seguir.}.

    \section{Conclusão}

     Resolvendo\footnote{Veja argumentação no Apêndice} a nova equação de recorrência obtida, obtemos que \( N(h) \geq 2^{\sfrac{h}{2}} \). Lembrando que \( N(h) \) é o número de nós de \( T \), que passaremos a denominar por \( n \), concluímos que, para todo \( h \geq 3 \),
     \begin{alignat*}{3}
       n = N(h) & \geq 2^{\sfrac{h}{2}} & \quad &  & \implies \\
       \log_2 n & \geq \frac{h}{2}      & \quad &  & \implies \\
       h        & \leq 2 \log_2 n       & \quad &  &
     \end{alignat*}

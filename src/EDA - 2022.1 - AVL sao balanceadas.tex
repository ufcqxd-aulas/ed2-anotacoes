    ...
    \subsection{Estrutura da árvore}
      ...
      \begin{figure}[hbt]
        ...
        \begin{tikzpicture}[transform shape, scale=.9, nodes={draw, circle, minimum width=3.5ex}, sibling distance=5.5em, level distance=6ex]
          \draw node [label=south:{\( 0 \)}] {\( n \)}
          child [child anchor=apex] { node [draw,isosceles triangle,shape border rotate=90,anchor=apex] (to replace) {\( n.esq \)}}
          child [child anchor=apex] { node [draw,isosceles triangle,shape border rotate=90,anchor=apex] {\( n.dir \)}}
          ;

          \node [right of=to replace] {\( \Rightarrow \)};
        \end{tikzpicture}
      \end{figure}

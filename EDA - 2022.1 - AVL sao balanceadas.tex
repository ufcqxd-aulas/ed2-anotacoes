...
De fato, para qualquer valor de \( h \), é o caso que \( N(h) \geq F(h) \). Vamos usar a fórmula fechada \( f(h) \) para essa sequência para obtermos uma relação entre \( h \) e o número de nós \( N(h) \) da árvore, que denotaremos simplesmente por \( n \). \[ f(h) = \dots \]
